...
public class Evaluator implements Cloneable
{
    ...
    public int Evaluate(StringTokenizer tokens, String FirstToken) throws Exception
    {
           int errcode = 0;
           boolean error = false;
           String Expr = "";
           ...
           if(!error)
           {
              ...
              else
              {
                 ...
                 if(!error)
                 {
                    errcode = organizeValueStack(tokens, VarType);

                    if(errcode == 0)
                    {
                       makePostFix();
                       result = calculate();
                       currentSymbolTable.setNewValue(VarName, currentSymbolTable.typeOfArgument(result), result);
		       //doing the typeOfArgument() thing to set the type for a variable of undef type.
                    }
                 }
    ...
    public int validateExpression(StringTokenizer tokens, String ResultType) throws Exception
    {
           boolean error = false;
           int errcode = 0;
	   int failcount = 0;
           ...
           String VarType = "";
           String result = "";
           String[4] types = {"boolean", "int", "float", "double"};
           ...
           if(!error)
           {
              ...
              if(!error)
              {
	         if(ResultType.compareTo("all") == 0)
		 {
                    for(int i = 0; i < 4; i++)
		    {
		        errcode = organizeValueStack(tokens, types[i]);
		        failcount += (organizeValueStack(tokens, types[i]) > 0) ? 1 : 0;
		    }

		    errcode = (failcount < 4) ? 0 : errcode;
		 }

		 else
		 {
                    errcode = organizeValueStack(tokens, ResultType);
		 }
              }
           }
    ...
    private int organizeValueStack(StringTokenizer tokens, String ResultType) throws Exception
    {
            ...
            while(!error && tokens.hasMoreTokens())
            {
                  ...
                  else
                  {
                        ...
                        LastToken.compareTo("(") == 0)
                     {
                        ...
                        {
                           TokenType = currentSymbolTable.typeOfArgument(VarName);
                           ...
                           {
                              if((ResultType.compareTo("double") == 0 || ResultType.compareTo("undefined") == 0)  &&
			         ...
			         ResultType = TokenType;
                              ...
                              {
                                 if((ResultType.compareTo("float") == 0 || ResultType.compareTo("undefined") == 0)  &&
                                    TokenType.compareTo("float") ==0)
                                 {
				    ResultType = TokenType;
                                 ...
                                 {
                                    if((ResultType.compareTo("int") == 0 || ResultType.compareTo("undefined") == 0)  &&
                     ...
                     else
                     {
                        ...
                        {
                           TokenType = currentSymbolTable.typeOfArgument(Token);
    ...
    public String compute(String op1, String opr, String op2) throws Exception
    {
            String result = "";
            String VarType = "";

            VarType = currentSymbolTable.typeOfArgument(op1);
